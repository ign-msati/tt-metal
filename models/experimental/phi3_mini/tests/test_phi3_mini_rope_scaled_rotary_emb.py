...
def test_phi3_mini_rope_scaled_roatry_emb(batches: int = 1, seq_len: int = 384, device=None):
    ...
    # Run torch model
    torch_position_ids = torch.arange(0, seq_len, 1, dtype=torch.long).unsqueeze(0).repeat(batches, 1).float()
    torch_value_states = torch.arange(0, seq_len, 1, dtype=torch.long).float()
